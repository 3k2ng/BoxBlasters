...
void AAirStrike::Tick(const float DeltaTime)
{
	...
	if (IsValid(Arena) && IsValid(Bomber) && Movement.Size() > 1e-3F)
	{
		const float MovementSpeed = 0.5F * GBaseBomberSpeed * (1.F + static_cast<float>(Bomber->Speed) *
			GSpeedMultiplier);
		SetActorLocation(ClampAirStrike(GetActorLocation() + Movement.GetClampedToMaxSize(1.0f) * DeltaTime * MovementSpeed));
